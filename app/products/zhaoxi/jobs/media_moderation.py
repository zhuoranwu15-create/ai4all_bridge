...
logger = logging.getLogger("ai4all.zhaoxi.media_moderation")
...
def review_pending_media_job(
    *, limit: Optional[int] = None, force: bool = False
    ...
    """scheduler 注入用：把朝夕的下架回调绑到平台批处理上。"""
    return review_pending_media_batch(
        limit=limit, force=force, on_rejected=retire_content_for_rejected_media
    )
