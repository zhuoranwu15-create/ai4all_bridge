...
from app.products.mingchan.infrastructure import account_deletion_records

logger = logging.getLogger("ai4all.products.mingchan.account_deletion")
    ...
    *, platform_user_id: str
    ...
    with connect() as conn:
        rows = conn.execute(
            ...
            WHERE u.owner_platform_user_id = ? AND u.app_id = ? AND a.app_id = ?
            """,
            (
                platform_user_id,
                MINGCHAN_APP_ID,
...
def execute_account_deletion(
    *,
    ...
    account_ids = list_runtime_accounts_for_deletion(platform_user_id=platform_user_id)
    wiped: Dict[str, Any] = {
        "accounts_wiped": 0,
        "messages_deleted": 0,
        "sessions_deleted": 0,
    ...
    with connect() as conn:
        universe = conn.execute(
            "SELECT id FROM universes WHERE owner_platform_user_id = ? AND app_id = ?",
            (platform_user_id, MINGCHAN_APP_ID),
        ).fetchone()
        universe_id = str(universe["id"]) if universe is not None else None
        ...
        resident_wish_jobs = conn.execute(
            ...
            "(SELECT id FROM resident_wishes WHERE universe_id = ?)",
            (universe_id,),
        ).rowcount
        conn.execute(
        ...
        resident_wish_letters = conn.execute(
            ...
            "WHERE universe_id = ? AND source = 'wish'",
            (universe_id,),
        ).rowcount
        resident_wishes = conn.execute(
            "DELETE FROM resident_wishes WHERE universe_id = ?",
            (universe_id,),
        ).rowcount
        conn.execute(
        ...
        conn.execute(
            ...
            "AND app_id = ? AND source_type = 'generated' AND NOT EXISTS ("
        ...
        if universe is None:
            ...
            conn.execute(
                ...
                "(SELECT id FROM resident_lifecycle_events WHERE universe_id = ?)",
                (universe_id,),
            )
            lifecycle_events = conn.execute(
                "DELETE FROM resident_lifecycle_events WHERE universe_id = ?",
                (universe_id,),
            ).rowcount
        ...
        app_notifications = conn.execute(
            "DELETE FROM app_notifications WHERE platform_user_id = ? AND app_id = ?",
            (platform_user_id, MINGCHAN_APP_ID),
        ).rowcount
        notification_preferences_deleted = conn.execute(
            "DELETE FROM product_notification_preferences "
            "WHERE platform_user_id = ? AND app_id = ?",
            (platform_user_id, MINGCHAN_APP_ID),
        ).rowcount
        sessions_revoked = conn.execute(
            "DELETE FROM platform_user_sessions WHERE platform_user_id = ? AND app_id = ?",
            (platform_user_id, MINGCHAN_APP_ID),
        ).rowcount
        memberships_deleted = conn.execute(
            "DELETE FROM product_memberships WHERE platform_user_id = ? AND app_id = ?",
            (platform_user_id, MINGCHAN_APP_ID),
        ).rowcount
    ...
    return {
        ...
        **media,
        ...
        "notification_preferences_deleted": int(
            notification_preferences_deleted or 0
        ),
        "sessions_revoked": int(sessions_revoked or 0),
        "memberships_deleted": int(memberships_deleted or 0),
    }
...
def delete_account_now(
    *,
    ...
    if (
        ...
        and reason_code not in account_deletion_records.DELETION_REASON_CODES
    ):
    ...
    record = account_deletion_records.record_deletion_execution(
        platform_user_id=platform_user_id,
