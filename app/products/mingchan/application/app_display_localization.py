...
class MissionTemplateView(Protocol):
    """使命展示本地化所需的最小只读投影。"""
...
def localized_resident_options(
    catalog: Dict[str, Any], *, language: Optional[str] = None, app_id: str = MINGCHAN_APP_ID
) -> Dict[str, Any]:
...
def localized_report_options(
    payload: Dict[str, Any], *, language: Optional[str] = None, app_id: str = MINGCHAN_APP_ID
) -> Dict[str, Any]:
...
def localized_mission_template(
    template: MissionTemplateView, *, language: Optional[str] = None, app_id: str = MINGCHAN_APP_ID
) -> Dict[str, str]:
