...
from app.products.mingchan.application.wish import (
    MAX_WISH_TEXT_CHARS,
...
def _daily_max(config: Any) -> int:
    try:
        return int(getattr(config, "mingchan_wish_daily_max", 0) or 0)
                    ...
                    + timedelta(
                        seconds=max(
                            ...
                            int(
                                getattr(
                                    self.config,
                                    "mingchan_wish_job_lease_seconds",
                                    600,
                                )
                            ),
                        )
                        ...
                        + timedelta(
                            days=max(
                                ...
                                int(
                                    getattr(
                                        self.config,
                                        "mingchan_mailbox_letter_ttl_days",
                                        30,
                                    )
                                ),
                            )
