...
from app.products.mingchan.domain.companion_world.naming import naming_status
...
from app.products.mingchan.application.product_localization import product_default_language
from app.products.mingchan.application.companion_world_turn import (
    run_mingchan_companion_world_turn,
)
from app.products.mingchan.infrastructure.world_repository import (
...
logger = logging.getLogger("ai4all.products.mingchan.world")

router = APIRouter(tags=["companion-world"])
_world_registry: ProductRegistry = PRODUCTION_PRODUCT_REGISTRY
_world_config = settings
...
def configure_runtime(
    ...
    *,
    ...
    """为固定鸣蝉 World router 注入产品注册表和测试配置。"""

    global _world_registry, _world_config
    _world_registry = registry
    _world_config = settings if config is None else config
...
_MEDIA_PATH_PREFIX = f"{CANONICAL_API_PREFIX}/media"
...
# 两个固定挂载前缀只用于剥离 namespace；鸣蝉不接受旧 ``/v1/*`` App 路由。
_MOUNT_PREFIXES = tuple(
    sorted(
        (CANONICAL_API_PREFIX, PROXY_STRIPPED_API_PREFIX),
        key=len,
        reverse=True,
    )
)
...
def _require_world_session(
    ...
    if not bool(getattr(_world_config, "mingchan_p1_enabled", False)):
        raise CompanionWorldApiError("feature_disabled", 404)
    try:
        _world_registry.require_enabled(MINGCHAN_APP_ID)
    except ValueError:
        raise CompanionWorldApiError("feature_disabled", 404) from None
...
def _require_feed_session(
    ...
    if not bool(getattr(_world_config, "mingchan_feed_enabled", False)):
        raise CompanionWorldApiError("feature_disabled", 404)
...
def _service() -> CompanionWorldService:
    return CompanionWorldService(
        SqlCompanionWorldRepository(registry=_world_registry),
        language=product_default_language(),
    )


def _feed_service() -> CompanionWorldFeedService:
    return CompanionWorldFeedService(
        SqlCompanionWorldRepository(registry=_world_registry)
    )
...
def chat_media_enabled(kind: str) -> bool:
    ...
    if kind == MEDIA_KIND_IMAGE:
        return bool(getattr(_world_config, "mingchan_chat_image_enabled", False))
    if kind == MEDIA_KIND_VOICE:
        return bool(getattr(_world_config, "mingchan_chat_voice_enabled", False))
    return False
...
def feed_image_enabled() -> bool:
    ...
    return bool(getattr(_world_config, "mingchan_feed_image_enabled", False))
    ...
    *,
    ...
    for media_id in media_ids:
        ...
        item = build_feed_image_item(
            ...
            path_prefix=_MEDIA_PATH_PREFIX,
        )
        if item is not None:
            items.append(item)
    if not items:
...
def _turn_media_kwargs(
    *, asset: Optional[Dict[str, Any]], caption: str, platform_user_id: str
    ...
    """把一条待发送资产翻译成鸣蝉 resident turn 的媒体入参（D-2 / D-5）。
...
async def _validation_error_handler(request: Request, exc: RequestValidationError):
    if _is_companion_world_path(request.url.path):
        ...
        _no_store(response)
        return response
    from app.products.mingchan.api.product_errors import is_public_product_path
    from app.products.mingchan.application.product_localization import product_message
