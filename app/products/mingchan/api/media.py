...
from app.db import SessionPrincipal, get_universe_visit
...
from app.products.mingchan.api.world_contracts import WORLD_ERROR_RESPONSES, MediaUploadResponse
from app.time_utils import beijing_naive_now

logger = logging.getLogger("ai4all.products.mingchan.media")
...
def _image_upload_enabled() -> bool:
    ...
    return signing_configured() and bool(
        getattr(settings, "mingchan_chat_image_enabled", False)
        or getattr(settings, "mingchan_feed_image_enabled", False)
    )
...
def _voice_upload_enabled() -> bool:
    ...
    return signing_configured() and bool(
        getattr(settings, "mingchan_chat_voice_enabled", False)
    )
