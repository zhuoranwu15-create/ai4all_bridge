...
分层归属：本模块是鸣蝉 infrastructure persistence adapter，不属于共享 ``app.db``，也不允许作为
跨产品数据入口。领域与 application 经 repository 调用本模块；所有带产品参数的入口都只接受
``app_id=mingchan``。
"""
import json
import re
...
__all__ = [
    ...
    "UniverseOwnerProductConflictError",
    "get_universe",
    "lock_universe",
    ...
    "get_resident_draft_by_wish_request",
    "count_resident_drafts_since",
    ...
    "advance_conversation_read_cursor",
...
def _require_mingchan_app_id(app_id: str) -> None:
    ...
    if app_id != MINGCHAN_APP_ID:
        raise ValueError("Mingchan persistence only accepts app_id=mingchan")
    ...
    with _tx(conn) as tx:
        if universe_id:
            ...
            row = tx.execute(
                ...
                "WHERE owner_platform_user_id = ? AND app_id = ?",
                (owner_platform_user_id, expected_app_id),
            ).fetchone()
    ...
    with _tx(conn) as tx:
        tx.execute(
            ...
            INSERT INTO character_templates(
                id, app_id, source_type, owner_platform_user_id, name, avatar_ref, summary,
    ...
    *,
    ...
    with _tx(conn) as tx:
        row = tx.execute(
            ...
            (template_id, expected_app_id),
        ).fetchone()
    return dict(row) if row else None


def list_initial_character_templates(
    *, app_id: str = MINGCHAN_APP_ID, conn: Optional[Connection] = None
) -> List[Dict[str, Any]]:
    """返回指定产品 active 初始模板目录；完整性由领域层校验。"""
    _require_mingchan_app_id(app_id)
    with _tx(conn) as tx:
        rows = tx.execute(
            """
            ...
            WHERE app_id = ? AND status = 'active'
              AND initial_candidate_rank IS NOT NULL
    ...
    *,
    ...
    with _tx(conn) as tx:
        row = tx.execute(
            ...
            """,
            (template_id, app_id, owner_platform_user_id),
        ).fetchone()
    return dict(row) if row else None
...
def resolve_conversation_for_owner(
    *,
    ...
    """按 owner 和产品解析会话。"""
    _require_mingchan_app_id(expected_app_id)
    params: List[Any] = [conversation_id, owner_platform_user_id, expected_app_id]
    with _tx(conn) as tx:
        row = tx.execute(
            f"""
            SELECT c.id AS conversation_id, c.universe_id, c.resident_id,
                   c.owner_platform_user_id, c.runtime_account_id, c.state
            FROM ai_conversations c
            JOIN accounts a ON a.id = c.runtime_account_id
            WHERE c.id = ? AND c.owner_platform_user_id = ?
              AND a.app_id = ?
            """,
            tuple(params),
        ).fetchone()
...
def list_conversations_for_owner(
    *,
    ...
    """按 owner 列出会话；cursor 与 runtime account 都可固定到同一产品。"""
    _require_mingchan_app_id(expected_app_id)
    ...
    product_clause = "AND a.app_id = ?"
    params.append(expected_app_id)
    with _tx(conn) as tx:
        if cursor_conversation_id:
            ...
            cursor_params.append(expected_app_id)
...
def advance_conversation_read_cursor(
    *,
    ...
    with _tx(conn) as tx:
        row = tx.execute(
            f"""
            SELECT c.id, c.runtime_account_id, c.last_read_message_id
            FROM ai_conversations c
            JOIN accounts a ON a.id = c.runtime_account_id
            WHERE c.id = ? AND c.owner_platform_user_id = ?
              {product_clause}
...
def resolve_resident_memory_scope(
    *,
    ...
    """解析鸣蝉 resident/World，并同时校验 World 与 runtime account 归属。"""
    _require_mingchan_app_id(expected_app_id)
    with _tx(conn) as tx:
        row = tx.execute(
            """
            ...
            WHERE r.runtime_account_id = ? AND r.status IN ('active', 'offline')
              AND u.app_id = ? AND a.app_id = ?
            """,
            (runtime_account_id, expected_app_id, expected_app_id),
        ).fetchone()
    return dict(row) if row else None
...
def resolve_resident_proactive_scope(
    *, runtime_account_id: str, conn: Optional[Connection] = None
    ...
    """解析鸣蝉 resident 的主动触达角色；非鸣蝉 account 返回 None。"""
    with _tx(conn) as tx:
        row = tx.execute(
            """
            SELECT r.id AS resident_id, r.universe_id, r.status
            ...
            WHERE r.runtime_account_id = ? AND r.status IN ('active', 'offline')
              AND u.app_id = ? AND a.app_id = ?
            """,
            (runtime_account_id, MINGCHAN_APP_ID, MINGCHAN_APP_ID),
        ).fetchone()
    return dict(row) if row else None
...
def list_human_proactive_account_ids_for_user(
    *, platform_user_id: str, conn: Optional[Connection] = None
    ...
    """列出真人在鸣蝉 World 内的 active resident runtime accounts。"""
    ...
    with _tx(conn) as tx:
        rows = tx.execute(
            ...
            JOIN accounts a ON a.id = r.runtime_account_id
            WHERE u.owner_platform_user_id = ? AND u.app_id = ?
              AND r.status = 'active' AND a.app_id = ? AND a.status = 'active'
            ORDER BY r.runtime_account_id ASC
            """,
            (platform_user_id, MINGCHAN_APP_ID, MINGCHAN_APP_ID),
        ).fetchall()
...
def get_owner_last_inbound_at(
    *, platform_user_id: str, conn: Optional[Connection] = None
    ...
    """聚合鸣蝉 owner bindings 与 resident account 的最近真人入站时间。"""
            ...
            WHERE m.direction = 'inbound' AND m.role = 'user'
              AND m.account_id IN (
                  ...
                  WHERE u.owner_platform_user_id = ?
                    AND u.app_id = ? AND a.app_id = ? AND a.status = 'active'
              )
            """,
            (
                platform_user_id,
...
def list_universe_ids_for_memory_compact(
    *,
    ...
    """稳定分页列出鸣蝉内有 active L3 facts 的 World，并返回下一页游标。"""
