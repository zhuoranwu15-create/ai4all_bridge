...
import app.db as notification_db
import app.db as world_db
from app.bootstrap.product_registry import MINGCHAN_APP_ID
from app.config import settings
from app.products.mingchan.infrastructure import notification_preferences as me_settings
from app.products.mingchan.domain.companion_world.contracts import AppNotificationRecord
...
from app.products.mingchan.infrastructure.world_repository import resolve_human_proactive_scope
...
class AppInboxAdapter:
    ...
    def delivery_block_reason(self, runtime_account_id: str) -> Optional[str]:
        """返回鸣蝉 resident 当前不可投递原因；可投递时返回 ``None``。"""

        if not bool(getattr(settings, "mingchan_app_inbox_enabled", False)):
            return "app_inbox_disabled"
        scope = world_db.resolve_resident_memory_scope(
            runtime_account_id=runtime_account_id,
            expected_app_id=MINGCHAN_APP_ID,
        )
        if scope is None or scope.get("status") != "active":
            return "active_mingchan_resident_not_found"
        ...
        if not (
            ...
            return "active_mingchan_world_not_found"
        if not _resident_notifications_allowed(str(world["owner_platform_user_id"])):
            return "app_inbox_quiet_hours_preference"
        return None

    def can_deliver(self, runtime_account_id: str) -> bool:
        """仅 flag 开启、账号映射 active confirmed world、且主人未选安静模式时可入箱。"""
        return self.delivery_block_reason(runtime_account_id) is None
    ...
    def reserve_human(
        ...
        *,
        ...
        inbox_enabled = bool(
            getattr(settings, "mingchan_app_inbox_enabled", False)
        )
        human_enabled = bool(
            getattr(
                settings,
                "mingchan_app_only_human_proactive_enabled",
                False,
            )
        )
