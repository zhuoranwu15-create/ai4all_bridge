    ...
    *,
    ...
    with _tx(conn) as tx:
        row = tx.execute(
            ...
            WHERE n.id = ? AND n.platform_user_id = ?
            """,
            (notification_id, platform_user_id),
        ).fetchone()
...
def _delete_notification_ids(
    tx: Connection, *, platform_user_id: str, ids: List[str]
) -> int:
    ...
    cursor = tx.execute(
        f"DELETE FROM app_notifications WHERE platform_user_id = ? "
        f"AND id IN ({placeholders})",
        (platform_user_id, *ids),
    )
    ...
    *,
    ...
    deleted = int(
        tx.execute(
            ...
            WHERE platform_user_id = ? AND delivery_status IN ('visible', 'cancelled')
              AND expires_at IS NOT NULL AND expires_at <= ?
            """,
            (platform_user_id, now),
        ).rowcount
        or 0
    )
    count = int(
    ...
    read_rows = tx.execute(
        ...
        LIMIT ?
        """,
        (platform_user_id, now, *protected_params, overflow),
    ).fetchall()
    read_ids = [str(row["id"]) for row in read_rows]
    deleted += _delete_notification_ids(
        tx, platform_user_id=platform_user_id, ids=read_ids
    )
    ...
    with _notification_write_tx(conn) as tx:
        ...
        tx.execute(
            ...
            INSERT INTO app_notifications(
                id, platform_user_id, universe_id, resident_id, scope, category,
        ...
        row = tx.execute(
            "SELECT * FROM app_notifications WHERE id = ? AND platform_user_id = ?",
            (notification_id, platform_user_id),
        ).fetchone()
    if row is None:
...
def list_app_notifications(
    *,
    ...
    params: List[Any] = [platform_user_id, now]
    if unread_only:
...
def count_unread_app_notifications(
    *, platform_user_id: str, now: str, conn: Optional[Connection] = None
) -> int:
    ...
    *,
    ...
    with _notification_write_tx(conn) as tx:
        tx.execute(
            ...
            WHERE id = ? AND platform_user_id = ? AND delivery_status = 'visible'
              AND expires_at > ? AND read_at IS NULL
            """,
            (now, read_expires_at, notification_id, platform_user_id, now),
        )
        row = tx.execute(
            """
            SELECT * FROM app_notifications
            WHERE id = ? AND platform_user_id = ? AND delivery_status = 'visible'
              AND expires_at > ?
            """,
            (notification_id, platform_user_id, now),
        ).fetchone()
    return dict(row) if row else None
    ...
    with _notification_write_tx(conn) as tx:
        ...
        cursor = tx.execute(
            ...
            WHERE platform_user_id = ? AND delivery_status = 'visible'
              AND expires_at > ? AND read_at IS NULL
            """,
            (now, read_expires_at, platform_user_id, now),
        )
    ...
    *, now: str, limit: int = 100, max_visible: int = 200
    ...
    with _notification_write_tx(None) as tx:
        rows = tx.execute(
            """
            SELECT id FROM app_notifications
            WHERE delivery_status = 'reserved' AND claim_expires_at <= ?
        ...
        if ids:
            ...
            cursor = tx.execute(
                ...
                """,
                (now, cancelled_expires_at, now, *ids),
            )
    ...
    with _notification_write_tx(None) as tx:
        rows = tx.execute(
            ...
            WHERE delivery_status IN ('visible', 'cancelled')
              AND expires_at IS NOT NULL AND expires_at <= ?
    ...
    with _notification_write_tx(conn) as tx:
        ...
        tx.execute(
            ...
            SET delivery_status = 'cancelled', cancelled_at = ?, expires_at = ?,
                terminal_reason = 'claim_expired', claim_token = NULL,
                updated_at = strftime('%Y-%m-%d %H:%M:%S', datetime('now', '+8 hours'))
            WHERE platform_user_id = ? AND scope = 'human'
              AND delivery_status = 'reserved' AND claim_expires_at <= ?
            """,
            (now, cancelled_expires_at, platform_user_id, now),
        )
        ...
        if existing is not None:
            ...
            if result["delivery_status"] == "cancelled":
                live = tx.execute(
                    """
                    SELECT id FROM app_notifications
                    WHERE platform_user_id = ? AND scope = 'human' AND id <> ?
                      AND (
                    ...
                    LIMIT 1
                    """,
                    (platform_user_id, result["id"], visible_since, now),
                ).fetchone()
                if live is not None:
                    return None, False
                tx.execute(
        ...
        live = tx.execute(
            ...
            LIMIT 1
            """,
            (platform_user_id, visible_since, now),
        ).fetchone()
        if live is not None:
            return None, False
        ...
        tx.execute(
            """
            INSERT INTO app_notifications(
                id, platform_user_id, universe_id, scope, category,
                source_type, source_id, idempotency_key, request_fingerprint,
    ...
    with _notification_write_tx(conn) as tx:
        ...
        cursor = tx.execute(
            ...
            SET delivery_status = 'cancelled', cancelled_at = ?, expires_at = ?,
                ...
                updated_at = strftime('%Y-%m-%d %H:%M:%S', datetime('now', '+8 hours'))
            WHERE id = ? AND platform_user_id = ? AND scope = 'human'
    ...
    with _notification_write_tx(conn) as tx:
        ...
        if allow_speaker_reselection:
            ...
            row = tx.execute(
                ...
                """ + lock_suffix,
                (expected_resident_id, universe_id, platform_user_id),
            ).fetchone()
        ...
        if speaker is None:
            ...
            tx.execute(
                ...
                SET delivery_status = 'cancelled', cancelled_at = ?, expires_at = ?,
                    ...
                    updated_at = strftime('%Y-%m-%d %H:%M:%S', datetime('now', '+8 hours'))
                WHERE id = ? AND platform_user_id = ? AND delivery_status = 'reserved'
                  AND claim_token = ?
                """,
                (
                    now,
        ...
        cursor = tx.execute(
            ...
            SET resident_id = ?, delivery_status = 'visible', title = ?, body_text = ?,
                ...
                updated_at = strftime('%Y-%m-%d %H:%M:%S', datetime('now', '+8 hours'))
            WHERE id = ? AND platform_user_id = ? AND universe_id = ?
