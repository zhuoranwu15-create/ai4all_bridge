...
def build_mailbox_policy(config: Any = settings) -> MailboxPolicy:
    ...
    cooldown = int(config.companion_world_mailbox_delivery_cooldown_days)
    ttl = int(config.companion_world_mailbox_letter_ttl_days)
...
class CompanionWorldMailboxService:
    ...
    def __init__(self, *, policy: Optional[MailboxPolicy] = None) -> None:
