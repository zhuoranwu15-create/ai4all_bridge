...
from app.db import get_active_llm_provider_id, set_active_llm_provider_id
...
def _load_provider_state() -> tuple[list[LLMProviderConfig], Optional[str], LLMProviderConfig]:
    try:
        providers = list_llm_providers(settings)
    except ValueError as err:
        raise HTTPException(status_code=400, detail=str(err)) from err
    stored_active_id = get_active_llm_provider_id()
    active_id = stored_active_id or _settings_default_provider_id()
    by_id = {provider.id: provider for provider in providers}
    active_provider = by_id.get(active_id) or by_id.get(_settings_default_provider_id()) or providers[0]
    return providers, stored_active_id, active_provider


def _provider_for_admin(provider: LLMProviderConfig, *, active_id: str) -> dict:
...
def admin_list_llm_providers(_: None = Depends(verify_admin_auth)) -> dict:
    providers, stored_active_id, active_provider = _load_provider_state()
    return {
        "active_provider_id": active_provider.id,
        "stored_active_provider_id": stored_active_id,
        "default_provider_id": _settings_default_provider_id(),
        "active_provider": active_provider.redacted(),
        "providers": [
            _provider_for_admin(provider, active_id=active_provider.id)
...
def admin_set_active_llm_provider(
    ...
    providers, _, _ = _load_provider_state()
