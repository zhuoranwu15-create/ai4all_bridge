...
OpenClaw（`openclaw/openclaw.git`）是上游第三方仓库，我们无法把改动提交进去。本项目落地依赖的 4 处 OpenClaw 能力全部以**手术补丁**形式维护：3 个能写成稳定 unified diff 的存为 `*.patch`，第 4 个因打在带内容哈希的 core bundle 上、无法写稳定 diff，改以幂等脚本重放。
...
> 第 4 个补丁刻意不存为 `.patch`：core dist 文件名随版本变哈希（`get-reply-9dLyvuw9.js` → `BpFiu3Nn.js`），写死路径的 diff 每次升级都会失效。锚点字符串稳定，脚本按内容 grep 自动发现 bundle。
