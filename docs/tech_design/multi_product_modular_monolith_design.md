...
关联后端设计：
- [`architecture_overview.md`](../architecture_overview.md) §2 已确立四层概念模型与依赖方向，本文是其在「多产品」维度上的延伸。
- [`companion_world_3_0_refactor_design.md`](./companion_world_3_0_refactor_design.md)（D-01…D-14）：Agent Runtime 形态无关 + Companion World 产品领域层的既有重构，本文**叠加而非推翻**。
- [`identity_model_and_wechat_binding.md`](./identity_model_and_wechat_binding.md)、[`companion_world_account_model_reconciliation.md`](./companion_world_account_model_reconciliation.md)：真人身份与账号收敛模型。
- [`multi_product_modular_monolith_implementation_plan.md`](../plans/multi_product_modular_monolith_implementation_plan.md)：当前 Phase 1 的 MP-01…MP-06 精简工单；Fatetell 产品接入等待 PRD 后再拆。
...
> 与 `architecture_overview.md §2` 的关系：那里已有「接入与产品 API → 产品领域 → AgentRuntimePort → 平台服务」的四层。本文在「平台服务」和「产品领域」之间显式插入 `app_id` 作为强隔离维度，并把 Companion World 明确降为 zhaoxi 产品域的子领域。
