...
- **密钥来源**：DeepSeek key = AI4ALL `.env` 的 `LLM_API_KEY`（base=`api.deepseek.com`）；DashScope key = AI4ALL `.env` 的 `DASHSCOPE_API_KEY`。直接 shell 内同步，不另找。注意 TDAI 侧 `TDAI_LLM_MODEL` 固定 `deepseek-chat`，不跟随 AI4ALL 主对话模型（AI4ALL 已改 family×tier，active family=deepseek 的 pro 档在 aliyun1 是 `deepseek-v4-pro`；见 [LLM family×tier 设计](llm_family_tier_design.md)）。
- **出网（两机相反，务必实测）**：
