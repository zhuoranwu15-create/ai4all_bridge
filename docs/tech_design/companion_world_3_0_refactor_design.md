...
### 6.5 最低成本切口与顺序
- ~~**八字托管段**已结构化/工具管理/有独立 read/write/preserve 通道 → 改指 universe 存储，是 **L3 首刀**~~ **（作废 2026-07-21：八字托管段整体移除、八字降级为无工具 skill、出生信息走普通记忆；L3 首刀改由通用 `user_fact`/`user_preference` 承载，见 §12 落地说明）**。
...
@dataclass(frozen=True)
class MemoryEvent:                       # *
    fact_type: str                      # 枚举 M2-0 定稿：user_fact/preference(→L3) | relationship(→L2) | ...（bazi 于 2026-07-21 移除，八字降级为无工具 skill）
