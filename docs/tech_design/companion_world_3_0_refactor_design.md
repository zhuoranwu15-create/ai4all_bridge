...
更新时间：2026-07-23
状态：**M0/M1/M2-C 与 M3 代码已完成；M4-0 产品门与实现级规范已冻结，M4 runtime 尚未开始。** PR #46 仍为 Draft；P1/M3 flag 均默认关闭且尚未生产启用，M5 仍受 §10.9 产品门约束。含主动消息子系统改造（D-13 / §11）。本文同时作为后续开发的权威接手入口；冻结口径与当前实现偏差均以本文为准。
核查基线：当前开发环境 `feat/companion-world-m4`，暂从 M3 已验证提交 `01552c9` 切出；PR #46 尚未合并，因此为依赖 M3 的堆叠分支。
...
5. **[`../plans/companion_world_m4_implementation_plan.md`](../plans/companion_world_m4_implementation_plan.md)**：当前待执行计划；M4-0 已完成，下一批为 M4-1。
...
- 纯领域层：`app/domains/companion_world/{contracts,service,feed,notifications,memory_sink,l3_context}.py`。
- SQL/platform adapters：`app/platform/{companion_world_repository,companion_world_memory}.py`。
...
- 数据与迁移：`app/db/companion_world.py`、`app/db/notifications.py`、`app/db/_core.py`（当前 max migration = m0033；三张 M3 表均已有 default-off 调用方）。
...
1. **D-06 已收口（m0031 同批）**：Runtime adapter 仅保留形态无关 `send_turn`；conversation owner 解析继续由 World repository 完成，L3 DB 读取、产品渲染与 `ChannelTurnInput` 组装已移到 `app/platform/companion_world_turn.py`。`tests/test_layer_boundaries.py` 同时禁止 Runtime import `app.domains.*` / `app.db.companion_world`，并钉住不得恢复 World convenience method。
...
| **M4** Lifecycle + Mailbox | R4 | offline+farewell 原子事务、信箱 | ✅ §10.3/.4/.8 与 mailbox 已冻结；M4-0 完成 |
| **M5** Visit + Human Chat | R5 | 三 slot/高熵 code/ACL、真人分表 | §10.9 |

M2–M5 不并行，每阶段无下一阶段仍是完整可回滚体验。**M0/M1/M2-C 与 M3 已交付代码；P1/M3 保持默认关闭，生产启用仍受模板、backfill、客户端版本和现场对账发布闸约束。M4 产品门已清零，可按 M4 plan 进入 M4-1；M5 仍不得在 §10.9 未冻结前编码。**
...
**落地说明（M2-A 数据基座+端口骨架已交付 2026-07-20）**：M2 按「产品门」拆刀落地——本刀 = **净空第一刀（当时不触 §10.1/.2/.6 冻结门）**，只铺**数据基座 + 端口形状**，**零 turn 路径改动、零行为变更**。交付：①迁移 `m0025_companion_world_core`（实际并主干后为 m0028）+ `m0026_universe_memory_l3`（实际为 m0029）；②`app/db/companion_world.py` 底层 repo 原语；③`app/agent_runtime/ports.py` 四接缝形状。M2-C 的 §10.1/.2/.6 已于 2026-07-21 冻结；[`../plans/companion_world_m2c_implementation_plan.md`](../plans/companion_world_m2c_implementation_plan.md) 已执行完毕并归档，不再作为下一步工作计划。
...
- 生产开启前仍必须完成四模板 manifest、固定 cutoff backfill、客户端最低版本、客户端共享/legacy 豁免口径同步与双后端门禁；详见 [`../guides/admin_guide.md`](../guides/admin_guide.md#companion-world-p1-发布运行手册)。
- M3 前置收口门禁（2026-07-22）：unit 566 passed；SQLite 1434 passed / 8 skipped；PostgreSQL 1438 passed / 4 skipped；`git diff --check` 通过。

### M3（M3-0…M3-6 已完成）、M4（M4-0 已冻结）与 M5
...
- **M4**：M4-0 已冻结 cooldown/audit/safety、last-resident、不可逆纠错和 mailbox 策略；后续按 [`companion_world_m4_backend_spec.md`](./companion_world_m4_backend_spec.md) 实现 offline+farewell+read-only 原子事务与 letters 接受事务。
- **M5**：三 slot + 高熵 code + 绝对过期 + visit ACL；`human_conversations`/`human_messages` 分表（D-11）、到期只读、举报/拉黑。

### 关键风险与门槛

1. **产品冻结项**是各里程碑硬前置：M2-C、M3 与 M4 的对应 §10 门均已清零；M4 runtime 尚未开始，PR #46 尚未合并，先按 M4 plan 完成基线校准。M5 仍等待 §10.9。
