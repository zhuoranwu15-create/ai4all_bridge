...
- 异步结果补发：Web Search、ASR 或其他用户触发长任务完成后的结果投递。
...
| 异步结果补发 | 用户触发的长任务 | 不受主动触达总开关影响 | 默认不因 quiet hours 丢弃 | 不占主动消息日上限 | 不参与主动消息避让 | 任务本身按任务成本扣减 |
...
Web Search 等高耗时任务默认异步：先同步告知“后台正在工作”，任务完成后再补发完整结果。
