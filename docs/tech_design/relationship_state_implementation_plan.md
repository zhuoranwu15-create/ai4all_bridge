...
在 `app/db/user_meta.py` 增加确定性 helper：
...
- B1 统计 / 余额 helpers：
  - `app/db/user_meta.py`：`count_inbound_messages(account_id)`（inbound 总数，不排除当天，供 30 阈值）；`list_recent_inbound_message_dates(account_id, since_date)`（自 `since_date` 起有 inbound 的去重北京日期列表，供连续天数判断）。
...
- C1 prompt 模块 `app/prompts/user_meta_relationship.py`：
  - 复用枚举（`relationship_stage`/trust/growth 合法值，与 `app/db/user_meta.py` 的 `*_VALUES` 一致）。
