...
对应 PRD：[`docs/product/campaign_codes_prd.md`](../product/campaign_codes_prd.md)
...
## 2. `app/db/campaign.py`（新模块，参照 `app/db/mission.py` 风格）
...
新 router `app/routers/admin_campaigns.py`：
...
- `app/db/campaign.py` 单测：CRUD、`validate_campaign_code` 各分支（不存在 / 过期 / disabled / 正常）、归因写入唯一性、`mission_id`/`soul_preset_key` 白名单校验、`code` 字符集校验、`status` 枚举校验、`valid_from`/`expires_at` 格式与顺序校验、默认 3 个月有效期计算。
