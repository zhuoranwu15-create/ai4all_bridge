...
| 微信入站端点 | `app/routers/bridge.py:175` | 不动 |
...
| **主动路由选择（无能力过滤 + 按 last_seen 排序）**| `app/proactive/contract/common.py:32`（`_select_route`）+ `accounts.py:191`（排序）| **只选 `supports_proactive=True` 渠道（§8.3，Phase 0 硬需求）** |
| **主动投递无条件走微信网关** | `app/proactive/delivery/outbound.py:287,329`（`send_weixin_text` / `dispatch_proactive_text`）| **对 `supports_proactive=False` 渠道 fail-fast，不进网关（§8.3）** |
