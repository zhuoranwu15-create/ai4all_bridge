...
```
DYNAMIC_REMINDER_ENABLED=true                 # 总开关（履约走 dynamic 分支）
DYNAMIC_REMINDER_ACCOUNT_ALLOWLIST=aid_554754198   # 灰度账号（创建 dynamic 提醒的准入）
...
```
- **两机都配**：aliyun2 处理入站→创建 dynamic 提醒需 enabled+allowlist；调度机执行到期履约+出站分发。
- `create_reminder` 的 `fulfillment=dynamic` 受 `ctx.dynamic_reminder_enabled` + allowlist 门控（未准入则忽略该字段，退回 fixed 或提示不支持）。
...
| `app/db/proactive.py` | dynamic 提醒读写、run CRUD/claim/状态翻转/对账查询（均带 account_id） |
...
| `app/tools/reminder_handlers.py` | 创建时解析 topic/max_items → `content_meta_json`；确认回复给精确下次时间 |
| `app/proactive/obligations/reminders.py` | dispatch 按 fulfillment 分叉；dynamic 履约 + run 状态机 + 对账步骤 |
| `app/proactive/fulfillment/*`（新增） | 合成轮次履约器（专用 prompt + 可配工具集 tool loop + 护栏） |
| `app/config.py` / `.env.example` | 6 个灰度项 |
| `app/routers/debug.py` / `admin_proactive.py` | dynamic 提醒 run-once、run 记录查询 |
