...
> **2026-07-08 补充（已确认）**：产品侧通过独立自测 + 网络搜索确认，`context_token` 的服务端有效期就是 **24 小时**——这是微信的官方口径，不是本文档反推出来的估计值。本文第七节的探针数据（约 20h45m 成功、约 44h45m 失败）与这个数字完全吻合（20h45m < 24h 送达成功，44h45m > 24h 送达失败），此前"24 小时是保守阈值、真实窗口在 (21h,45h) 区间"的措辞已过时，改为直接采用 24 小时。已写入 [`docs/product/proactive_prd.md` §9](../product/proactive_prd.md)（产品级约束与影响）。本文档是该约束的技术权威来源，产品文档不重复技术细节，只做结论引用。
...
上面第 3 点"窗口很宽、未二分"已由产品侧独立确认解决：服务端有效期就是官方口径的 **24 小时**，落在本节探针给出的 (21h, 45h) 区间内、与两个数据点（20h45m 成功、44h45m 失败）不矛盾。第 2/4 点（`ret:-2` 是否与限速共用同一个码）**仍未解决**，与有效期数字本身是两件独立的事，跟踪在 [`主动消息送达窗口对齐.md`](../plans/主动消息送达窗口对齐.md) 的 P4。
