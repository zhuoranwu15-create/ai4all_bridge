...
| [搜索与异步任务](product/search_and_async_tasks_prd.md) | `web_search` 同步工具调用、DuckDuckGo/Bing RSS/Aliyun IQS/Baidu AI Search、异步兜底、成本明细 | [搜索与异步任务技术设计](tech_design/search_async_tasks_design.md)、[Conversation Orchestrator 主对话场景技术设计](tech_design/conversation_orchestrator_design.md) | 暂无正式模块 | 待实现 | tool registry、`web_search` handler、搜索 provider adapter、tool invocation trace、异步兜底任务、补发幂等、引用格式、失败扣减规则、贝壳成本事件 |
...
| 长任务体验 | 普通 Web Search 默认同步 tool use；长耗时搜索、复杂整理和语音可按耗时转异步，先确认再补发 | [搜索 PRD](product/search_and_async_tasks_prd.md)、[语音 PRD](product/voice_prd.md)、[搜索技术设计](tech_design/search_async_tasks_design.md)、[语音技术设计](tech_design/voice_input_design.md)、[Conversation Orchestrator](tech_design/conversation_orchestrator_design.md) | tool invocation trace、`tasks`/`task_runs`、worker、provider adapter、outbound result delivery |
...
5. [搜索与异步任务技术设计](tech_design/search_async_tasks_design.md)：已按 Web Search 同步 tool use、provider 回退、异步兜底、结果补发和成本事件更新；后续进入代码实现。
...
6. 工具与异步任务：`web_search` tool schema、provider adapter、tool invocation trace、`tasks`/`task_runs`、ASR provider、结果补发。
