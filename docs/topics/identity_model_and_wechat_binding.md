...
- 已在插件源码工作区和本机运行时安装包补充 `gatewayMethods: ["web.login.start", "web.login.wait"]`，重启 Gateway 后，`web.login.start` 已能返回 `qrDataUrl`、`message`、`sessionKey`。补丁维护说明见 `docs/topics/openclaw_weixin_gateway_qr_patch.md`。
