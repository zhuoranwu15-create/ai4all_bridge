...
> 仓库状态（2026-08-04）：鸣蝉服务端契约已完成代码迁移，但生产注册仍禁用；本文给出发布目标
> 接口。客户端必须清除旧朝夕 App token/缓存后重新注册，不得复用 legacy 状态。

## 1. 接入基础

| 项 | 值 |
|---|---|
| 生产 Base URL（推荐） | `https://ai4company.top/api/v1/products/mingchan/` |
...
两个前缀挂的是**同一套路由、同一套行为**。`/api/v1/products/mingchan` 是规范命名空间，
`/v1/products/mingchan` 仅用于反代剥离兼容；旧 `/v1/*` 不属于鸣蝉。
...
信箱、访问/邀请、站内通知、真人会话、媒体、图文 Feed、「我的」Tab 与异步许愿已在生产开启；
异步许愿不设独立开关，随 resident world + mailbox 可用，客户端仍必须按 `/app/config` 的
能力位渲染。端点、字段、状态机和错误码只在
...
- 读媒体用返回的**短 TTL 签名 URL**，**不带 `Authorization`、不要持久化**；`url=null` 渲染占位。
  该 URL 是 **origin 相对路径**（`/api/v1/products/mingchan/media/…?scope=&exp=&sig=`）：拼 `https://ai4company.top` 即可，
  **不要再次拼 base URL**（返回值已经包含鸣蝉产品前缀）。
