# 鸣蝉 App 端接入交接文档（Companion World 3.0）

> 面向：鸣蝉移动端 App 开发者。
> 仓库状态（2026-08-04）：服务端契约和代码已迁入 `product:mingchan`，固定 `app_id=mingchan`；
> 生产注册仍保持禁用。本文件描述鸣蝉发布目标契约，不表示生产地址当前已经开放。
> 拆分前朝夕 App 的生产实测与开量记录仅作历史基线；新客户端不得复用旧朝夕 token、缓存或
> `/v1/*` legacy 路由。发布与真机验证按拆分计划 MC-07 执行。

---

## 1. 一句话架构（App 开发必读）

- App 只对接中心节点的鸣蝉产品 API，规范公网入口为
  `https://ai4company.top/api/v1/products/mingchan/`。
- 反代剥离兼容入口是 `/v1/products/mingchan/*`；不提供旧 `/v1/*` App 路由兼容。
- 微信/OpenClaw 属于朝夕，Native App 属于鸣蝉；两产品 session audience 和 runtime account 隔离。
...
| 项 | 值 |
|---|---|
| 公网 Base URL（新客户端推荐） | `https://ai4company.top/api/v1/products/mingchan/` |
| 反代剥离兼容 Base URL | `https://ai4company.top/v1/products/mingchan/` |
...
服务端将同一组鸣蝉路由挂到规范产品命名空间与反代剥离兼容前缀。新客户端一律使用规范前缀，
例如 `POST https://ai4company.top/api/v1/products/mingchan/auth/otp/send`。Base URL 必须集中配置，
不能在业务代码中散落硬编码。
...
仓库提交了客户端契约的 OpenAPI 快照：[`openapi/app_v1.json`](openapi/app_v1.json)。路径与操作数量
以快照为准，不在手写文档中重复维护。
...
```
GET  /api/v1/products/mingchan/app/config                → 拿产品默认语言 + captcha 配置 + 功能开关 + 限额
POST /api/v1/products/mingchan/auth/otp/send             → 发送短信验证码（需先过阿里云行为验证码）
POST /api/v1/products/mingchan/auth/otp/verify           → 校验 OTP，拿一次性 verified_token
POST /api/v1/products/mingchan/auth/session              → 用 verified_token 建立 30 天 session
   ↳ 拿到 access_token，后续所有请求带 Authorization: Bearer <access_token>
DELETE /api/v1/products/mingchan/auth/session/current    → 登出（吊销当前 token）
```

### 3.1 `GET /api/v1/products/mingchan/app/config`（无需鉴权）

线上实测返回（2026-07-31 只读核验；以下仅截取能力位与限额结构，客户端仍必须逐次按真实响应渲染）：

```json
{
  "product": { "app_id": "mingchan", "default_language": "zh-CN" },
...
### 3.2 `POST /api/v1/products/mingchan/auth/otp/send`

请求体：手机号 + 阿里云验证码校验参数。返回 `{ "status": "ok" }` 或错误。

### 3.3 `POST /api/v1/products/mingchan/auth/otp/verify`
...
### 3.4 `POST /api/v1/products/mingchan/auth/session`
...
### 3.5 `GET /api/v1/products/mingchan/me`（Session 恢复入口）
...
- `GET /api/v1/products/mingchan/me/profile-options` → `{"status":"ok","avatars":[{"key":"user_01","avatar_ref":"…"}],
  "limits":{"nickname_chars":20,"nickname_min_chars":1}}`。头像库由服务端下发，**客户端不要
  硬编码枚举**；数组为空时隐藏头像选择器。
- `PATCH /api/v1/products/mingchan/me/profile`，body `{"display_name"?, "avatar_key"?}`。两个字段都可单独提交，
...
- 只有一个端点：`POST /api/v1/products/mingchan/me/account/deletion`，body `{"confirm": true, "reason_code"?}`。
...
- `GET /api/v1/products/mingchan/notifications/preferences` → `data: {"quiet_level":"standard","available_levels":["standard","quiet"]}`。
  从未设置过时返回默认值，读不写库。
- `PATCH /api/v1/products/mingchan/notifications/preferences`，body `{"quiet_level":"quiet"}`，幂等。取值不在
  `available_levels` 内 → `422 quiet_level_invalid`。
- `quiet` 只压制**将来**的 AI 主动通知；**已在箱内的通知不回收**，`unread_count` 不会因为
  改偏好而变化。切回 `standard` 后立即恢复投递。
- 与 `/api/v1/products/mingchan/notifications` 一样需要 `COMPANION_WORLD_APP_INBOX_ENABLED`，否则 `404 feature_disabled`。
- **作用域仅限 AI 主动通知，不覆盖真人会话**（M5-NOTIFY-001）。服务端目前既没有真人消息
  Push 通道，也没有会话级静音字段/端点，因此 `quiet_level` 不能被解释成「真人会话免打扰」。
  产品已选择[方案 1：明确后置](../../backlog/products/mingchan/companion_world_app_followups.md)：
...
```
POST /api/v1/products/mingchan/worlds/home/bootstrap          → 创建/获取家园世界 + 返回候选居民
GET  /api/v1/products/mingchan/worlds/home/resident-candidates → （可重新拉取候选）
POST /api/v1/products/mingchan/worlds/home/residents/confirm  → 确认选定的居民，正式入住
GET  /api/v1/products/mingchan/worlds/home/residents          → 列出已入住居民（含会话 id）

# 自建角色（两步，见 §5.6）
GET  /api/v1/products/mingchan/worlds/home/resident-options            → 受控取值表（关系/性格/头像）
POST /api/v1/products/mingchan/worlds/home/resident-drafts/preview     → 预览人设，拿 draft_token
POST /api/v1/products/mingchan/worlds/home/residents                   → 用 draft_token 落地
```

### 5.1 `POST /api/v1/products/mingchan/worlds/home/bootstrap`
...
**第 0 步 `GET /api/v1/products/mingchan/worlds/home/resident-options`** —— 取值表以服务端为准，**客户端不要硬编码枚举**：
...
**第 1 步 `POST /api/v1/products/mingchan/worlds/home/resident-drafts/preview`**：
...
**第 2 步 `POST /api/v1/products/mingchan/worlds/home/residents`**（同一端点也用于加入预设模板）：
...
```
GET  /api/v1/products/mingchan/conversations                              → 会话列表
GET  /api/v1/products/mingchan/ai-conversations/{conversation_id}/messages → 历史消息
POST /api/v1/products/mingchan/ai-conversations/{conversation_id}/turn     → 发一条消息，拿 AI 回复
POST /api/v1/products/mingchan/ai-conversations/{conversation_id}/read     → 标记已读到某条消息
```
...
- **`media_ref`（v1.5 新增，可选）**：先走 `POST /api/v1/products/mingchan/media/uploads` 拿到的 `media_id`，发图片或语音时带上，
...
```
GET    /api/v1/products/mingchan/worlds/home/feed?cursor=<游标>&limit=20        → 分页拉取 Feed（limit 1-50，默认 20）
POST   /api/v1/products/mingchan/worlds/home/feed/posts                         → 用户发帖
DELETE /api/v1/products/mingchan/worlds/home/feed/posts/{post_id}               → 主人删除自己的动态
POST   /api/v1/products/mingchan/worlds/home/feed/posts/{post_id}/hide          → 主人隐藏 AI 居民动态
```
...
- **不存在、跨 owner、不可见、post_id 格式非法统一返回 404 `post_not_found`**，不区分「没有」
  和「不是你的」，避免资源枚举。
- **隐藏是服务端权威状态，不是单设备偏好**：主人和所有有效访客（`GET /api/v1/products/mingchan/visits/{id}/feed`）
...
**`GET /api/v1/products/mingchan/human-conversations`** 的每个 item 补齐了以下字段，客户端不必再自行 join
`/visits` 或伪造未读 badge：
...
**`GET /api/v1/products/mingchan/human-conversations/report-options`**（新增）返回版本化举报原因表：
...
```
POST /api/v1/products/mingchan/media/uploads                 → 上传，拿 media_id + 一条短 TTL 读 URL
GET  https://ai4company.top/api/v1/products/mingchan/media/{media_id}?scope=&exp=&sig=
...
```json
{
  ...
  "transcript": null,
  "url": "https://ai4company.top/api/v1/products/mingchan/media/med_…?scope=pu:…&exp=…&sig=…",
...
- **`url` 是客户端可直接加载的完整 HTTPS URL**，canonical path 为
  `/api/v1/products/mingchan/media/{media_id}`；客户端不要再拼 origin、API base 或产品 namespace。
- **不要持久化、不要跨会话复用、不要写进本地库**。过期就重新拉一次列表/详情拿新签名。
- `GET /api/v1/products/mingchan/media/{media_id}` **不读 `Authorization`**，签名三元组
  （`scope` / `exp` / `sig`）就是唯一凭据；
  原样使用返回的 URL，不要自己拼参数。
- **`url` 为 `null` 表示服务端此刻签不出**（部署缺 secret，或访客的拜访已结束）：
  **按占位图渲染，不要降级成文本、不要当成消息损坏**。
- 访客视角（`GET /api/v1/products/mingchan/visits/{visit_id}/feed`）拿到的是按 visit scope 签的 URL，**拜访一结束立即失效**。
...
```
POST /api/v1/products/mingchan/ai-conversations/{conversation_id}/turn
POST /api/v1/products/mingchan/human-conversations/{conversation_id}/messages
...
```
POST /api/v1/products/mingchan/worlds/home/feed/posts
...
`POST /api/v1/products/mingchan/me/account/deletion` 现在会连带删除该用户的媒体库行与磁盘文件、以及自己世界的全部动态。
...
```
POST /api/v1/products/mingchan/worlds/home/resident-wishes             { "wish_text": "…", "client_request_id": "…" }
GET  /api/v1/products/mingchan/worlds/home/resident-wishes/current
POST /api/v1/products/mingchan/resident-wishes/{wish_id}/withdraw      {}
```
...
> 完整表见 `app/products/mingchan/api/world.py` 的 `_ERROR_STATUS`。App 应基于 `code`（而非文案）做分支，未知 `code` 按对应 HTTP 状态兜底。
...
**鉴权 / 账户**
- `GET /api/v1/products/mingchan/app/config`
- `POST /api/v1/products/mingchan/auth/otp/send`、`POST /api/v1/products/mingchan/auth/otp/verify`
- `POST /api/v1/products/mingchan/auth/session`、`DELETE /api/v1/products/mingchan/auth/session/current`
- `GET /api/v1/products/mingchan/me`
- `GET /api/v1/products/mingchan/me/profile-options`、`PATCH /api/v1/products/mingchan/me/profile`
- `POST /api/v1/products/mingchan/me/account/deletion`（立即注销，不可撤销）

**主聊天（默认账号，非世界）**
- `GET /api/v1/products/mingchan/chat/messages`、`POST /api/v1/products/mingchan/chat/turn`
- `POST /api/v1/products/mingchan/audio/transcriptions`（语音转写；能力位由所选 ASR provider 的完整凭据决定）
...
**世界 / 家园**
- `POST /api/v1/products/mingchan/worlds/home/bootstrap`
- `GET /api/v1/products/mingchan/worlds/home/resident-candidates`、`GET /api/v1/products/mingchan/worlds/home/resident-options`
- `POST /api/v1/products/mingchan/worlds/home/resident-drafts/preview`
- `POST /api/v1/products/mingchan/worlds/home/resident-wishes`、`GET /api/v1/products/mingchan/worlds/home/resident-wishes/current`
- `POST /api/v1/products/mingchan/resident-wishes/{wish_id}/withdraw`
- `GET /api/v1/products/mingchan/worlds/home/residents`、`POST /api/v1/products/mingchan/worlds/home/residents`、`POST /api/v1/products/mingchan/worlds/home/residents/confirm`
- `GET /api/v1/products/mingchan/worlds/home/feed`、`POST /api/v1/products/mingchan/worlds/home/feed/posts`
- `DELETE /api/v1/products/mingchan/worlds/home/feed/posts/{id}`、`POST /api/v1/products/mingchan/worlds/home/feed/posts/{id}/hide`

**世界内会话**
- `GET /api/v1/products/mingchan/conversations`
- `GET /api/v1/products/mingchan/ai-conversations/{id}/messages`、`POST /api/v1/products/mingchan/ai-conversations/{id}/turn`
- `POST /api/v1/products/mingchan/ai-conversations/{id}/read`

**信箱**
- `GET /api/v1/products/mingchan/mailbox/letters`、`GET /api/v1/products/mingchan/mailbox/letters/{id}`、`GET /api/v1/products/mingchan/mailbox/unread-count`
- `POST /api/v1/products/mingchan/mailbox/letters/{id}/read`、`/accept`、`/decline`、`/defer`

**访问 / 邀请**
- `GET /api/v1/products/mingchan/visits`、`POST /api/v1/products/mingchan/visits/redeem`
- `GET /api/v1/products/mingchan/visits/{id}/feed`
- `POST /api/v1/products/mingchan/visits/{id}/accept`、`/reject`、`/cancel`、`/leave`、`/revoke`、`/block`
- `POST /api/v1/products/mingchan/world/invites`、`GET /api/v1/products/mingchan/world/invites`、`DELETE /api/v1/products/mingchan/world/invites/{id}`

**通知**
- `GET /api/v1/products/mingchan/notifications`、`GET /api/v1/products/mingchan/notifications/unread-count`
- `POST /api/v1/products/mingchan/notifications/{id}/read`、`POST /api/v1/products/mingchan/notifications/read-all`
- `GET /api/v1/products/mingchan/notifications/preferences`、`PATCH /api/v1/products/mingchan/notifications/preferences`

**媒体（v1.5）**
- `POST /api/v1/products/mingchan/media/uploads`（图片 / 语音上传，multipart）
- `GET /api/v1/products/mingchan/media/{media_id}?scope=&exp=&sig=`（公网签名读，不带 `Authorization`）

**真人会话**
- `GET /api/v1/products/mingchan/human-conversations`
- `GET /api/v1/products/mingchan/human-conversations/report-options`
- `GET /api/v1/products/mingchan/human-conversations/{id}/messages`、`POST /api/v1/products/mingchan/human-conversations/{id}/messages`
- `POST /api/v1/products/mingchan/human-conversations/{id}/read`、`/report`、`/block`
- `DELETE /api/v1/products/mingchan/human-conversations/{id}/entry`
...
1. 先打 `GET /api/v1/products/mingchan/app/config` 确认连通与 captcha 参数。
