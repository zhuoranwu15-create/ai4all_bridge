...
本文承接 [运营与后台 PRD](admin_ops_prd.md)，只描述运营视角下的后台页面、字段、动作和优先级。技术 Debug 页面和命令统一收口到 [调试指南](../../../ops/products/zhaoxi/debugging.md)，不在这里展开。
...
开发阶段为了效率，可以在开发机通过环境变量开启明文 Debug；该策略只作为开发便利，不改变生产验收标准。具体约定见 [调试指南](../../../ops/products/zhaoxi/debugging.md#开发期明文策略)。
