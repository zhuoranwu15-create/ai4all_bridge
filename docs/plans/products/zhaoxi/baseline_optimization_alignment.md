...
> [`../backlog/100k_dau_scaling_review.md`](../../../backlog/shared/100k_dau_scaling_review.md)。本文不重复。
...
**补修（2026-06-16 收尾复审后）**：P0-1 当时只扫了 `app/products/zhaoxi/proactive/*` 的调度 dispatcher，漏了
`app/products/zhaoxi/tools/content_invitation_handlers.py`——该文件 6 处 `datetime.now()` 既写 DB 北京时间列
...
- 2026-06-16：**补修 P0-1 漏网点**（Codex 复审提出）。`app/products/zhaoxi/tools/content_invitation_handlers.py` 6 处 `datetime.now()` → `beijing_naive_now()`，统一到北京时区不变量（含活跃窗口/过期比较两处真实潜在 bug + 4 处落库时间戳口径）。同步全量审计了 `app/` 其余 `datetime.now()` 调用方，归类记于上方「宿主机时区不变量·补修」段：另有 `session_lifecycle.py:61`、`db/ops.py:39` 为同类待定项，纯展示/耗时类无需改。630 测试全绿。
