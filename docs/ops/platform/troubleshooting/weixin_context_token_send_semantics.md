...
> **2026-07-08 补充（已确认）**：产品侧通过独立自测 + 网络搜索确认，`context_token` 的服务端有效期就是 **24 小时**——这是微信的官方口径，不是本文档反推出来的估计值。本文第七节的探针数据（约 20h45m 成功、约 44h45m 失败）与这个数字完全吻合（20h45m < 24h 送达成功，44h45m > 24h 送达失败），此前"24 小时是保守阈值、真实窗口在 (21h,45h) 区间"的措辞已过时，改为直接采用 24 小时。已写入 [`docs/products/zhaoxi/capabilities/proactive_prd.md` §9](../../../products/zhaoxi/capabilities/proactive_prd.md)（产品级约束与影响）。本文档是该约束的技术权威来源，产品文档不重复技术细节，只做结论引用。
...
- **本次审计+探针未精确解出的部分（仅代码层面）**：context_token 服务端有效期的**具体时长**（07-03 实验只证明"1 个月的旧 token 已失效"，未二分窗口边界）；以及 07-02 样本里 `ret:-2` 占 76% 且含活跃会话，**限速语义是否与"token 陈旧拒收"共用 -2** 仍需分离（要拿日志级 `ret` 须重新打诊断补丁）——这条独立于有效期数字本身，仍未解决，见 §9 讨论。
- **阶段性缩窄（2026-07-05，单账号 n=1，弱证据）**：`probe_context_token_ttl.py` 每日探针给出窗口的首个数据点——同一枚 context_token（07-03 13:15 刷新后未变），放置 **~20.75h 仍成功送达**、**~44.75h 已翻转为 `ret:-2` 失败**。即服务端有效期落在 **(约 21h, 约 45h)** 区间，与官方口径的 24 小时完全吻合（见下）。
- **2026-07-08 已确认（官方口径）**：产品侧独立自测 + 网络搜索确认，服务端有效期就是 **24 小时**，不再是估计值。与上面探针的 (21h, 45h) 区间一致，是该区间内的确切数字。本文档及下游 `proactive_prd.md`/`主动消息送达窗口对齐.md` 已按 24 小时更新。
...
上面第 3 点"窗口很宽、未二分"已由产品侧独立确认解决：服务端有效期就是官方口径的 **24 小时**，落在本节探针给出的 (21h, 45h) 区间内、与两个数据点（20h45m 成功、44h45m 失败）不矛盾。第 2/4 点（`ret:-2` 是否与限速共用同一个码）**仍未解决**，与有效期数字本身是两件独立的事，跟踪在 [`主动消息送达窗口对齐.md`](../../../plans/products/zhaoxi/主动消息送达窗口对齐.md) 的 P4。
