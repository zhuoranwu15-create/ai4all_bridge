...
- **归档**（`git mv` 至 `docs/archive/`，保留历史）：
  ...
  - `docs/tech_design/timezone_refactor_plan.md`（已完成的时区统一重构计划）→ `docs/archive/phase1/`。
...
- **保留未动**（仍被多份 live 文档引用或仍具前瞻价值，仅在此标注）：
  - `docs/tech_design/voice_input_asr_tracking.md`：被 traceability、voice_prd、voice_input_design 引用，留原处。
  - `docs/tech_design/{agent_orchestration_roadmap,production_stability_prd}.md`、`docs/product/{website_prd,admin_ops_views}.md`：Phase 1 范围内前瞻/运营规划，进入下一阶段时再评估归档。
