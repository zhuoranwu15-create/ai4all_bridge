...
最初作为朝夕 Native App 开发的 Companion World 引入了更丰富的业务：一个真人可以拥有私人
世界和多位居民，每位居民仍通过底层 Agent Runtime 与用户聊天，同时增加 Feed、通知、信箱、
生命周期、访问和真人会话。2026-08-04 决定不再把它视为朝夕的一种形态，而是拆为独立产品
“鸣蝉”（`app_id=mingchan`）。这个变化促使后端重构为：
...
不能把鸣蝉的 Companion World/居民或朝夕的主动消息假设当成所有产品的默认模型。
...
| 产品（product） | 有独立业务目标、`app_id`、membership、契约和产品 owner 的业务容器 | 朝夕相伴 `zhaoxi`、鸣蝉 `mingchan` | 微信、iOS 或一个页面 |
| 业务形态（experience/form） | 同一产品内围绕核心价值形成的一套用户体验和领域组合 | 朝夕的微信陪伴、鸣蝉的 Native App Companion World | 自动获得独立数据边界的新产品 |
| 渠道（channel） | 消息或操作进入/离开系统的协议与投递能力 | WeChat/OpenClaw、Web/H5、Native App | 产品领域本身 |
| 产品领域（product domain） | 只有该产品理解的业务概念和规则 | 鸣蝉的 universe、resident、relationship、mailbox | 可直接下沉 Runtime 的通用概念 |

当前生产注册表启用 `zhaoxi`，并预注册但禁用 `mingchan`；鸣蝉必须在 App/World 代码与数据作用域
完成拆分后才可启用。其他候选产品只有在核心 PRD 冻结后才能创建自己的产品 manifest、代码命名
空间和生产入口。
...
| `universe_id` | 鸣蝉私人世界 | Companion World 的居民集合、L3 typed facts、Feed、通知、信箱与访问 | 这是鸣蝉产品锚点，不能作为通用 Platform/Runtime 主键 |
| `resident_id` | 鸣蝉世界中的居民实体 | 将产品居民映射到独立 runtime account，并承载生命周期等领域状态 | resident 与 runtime account 不能混成同一领域对象 |
...
2. 一个真人在鸣蝉中可以拥有多位居民，因此 `platform_user_id` 不等于 runtime `account_id`。
...
| Platform | `app/platform/`、共享 DB/repository | 身份、鉴权、配额、钱包、审核、媒体、网关、观测 | 不依赖任何 `app.products.*` 实现 |
...
微信渠道可以由朝夕 API adapter 直接组合 Runtime；Companion World 必须先由鸣蝉产品域解析
...
- 朝夕微信陪伴领域写入 `architecture/products/zhaoxi/`；鸣蝉 Companion World 领域写入
  `architecture/products/mingchan/`。
- 用户价值、流程和验收写入 `products/<app_id>/`，不能用技术设计替代 PRD。
- 某渠道上的产品体验仍归对应产品；只有协议、接入节点或共享认证属于 shared。
- 当前进度只写 `STATUS.md`，完成计划回写稳定事实后归档。

判断一个新能力应放在哪里时，先问：朝夕与鸣蝉是否都能不改变业务语义地使用？能原样使用的
候选 shared/Runtime；需要微信陪伴语义的留在 `product:zhaoxi`，需要 World/居民语义的留在
`product:mingchan`。
