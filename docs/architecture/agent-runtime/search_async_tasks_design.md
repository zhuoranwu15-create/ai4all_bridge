...
本文承接 [Web Search 同步工具调用 PRD](../../products/zhaoxi/capabilities/search_and_async_tasks_prd.md)，定义 Phase 1 `web_search` 工具、搜索 provider、同步边界、trace、成本事件和失败体验。

语音输入有独立技术文档，见 [语音输入技术设计](../products/zhaoxi/voice_input_design.md)。当前语音依赖上游转写，不进入后端 task/ASR 成本链路。Web Search 的 Phase 1 链路是同步 LLM tool use。
...
- 微信语音上游转写口径，见 [语音输入技术设计](../products/zhaoxi/voice_input_design.md)。
- 主动消息策略，见 [主动消息与提醒设计](../products/zhaoxi/proactive_messaging_design.md)。
- 贝壳 wallet/ledger 细则，见 [贝壳、增长与支付后置技术设计](../shared/platform/entitlement_growth_design.md)。
