...
本文承接 [陪伴式聊天 PRD](../../products/zhaoxi/capabilities/companion_chat_prd.md) 和 [记忆与上下文 PRD](../../products/zhaoxi/capabilities/memory_prd.md)，定义 AI4ALL Phase 1 的账号级 Context Files、短期上下文、daily notes 和长期记忆文件边界。Dreaming 详细设计见 [Dreaming 记忆压缩与长期记忆技术设计](dreaming_memory_design.md)；检索式记忆后续单独拆文档。
...
| `IDENTITY.md` | AI 对外身份、名字、是否暴露 OpenClaw 的回答边界 | 默认模板、首次聊天 onboarding、留白后的自然养成 |
| `SOUL.md` | AI 人格、语气、陪伴风格 | 默认模板、首次聊天 onboarding、留白后的自然养成、用户显式风格偏好 |
...
### 4.1 首次聊天 onboarding 写入口径

首次聊天 onboarding 当前是两步流程：

1. 第一步询问用户希望 AI 如何称呼用户；
2. 第二步合并询问用户想如何称呼 AI，以及希望 AI 是什么人设。

用户回复的解析主路径是 Conversation Orchestrator 中的专用 LLM extraction，不使用关键词或正则作为主要判断方式。Context Files 只接收通过 schema 校验和置信度校验后的结构化结果。

写入规则：

| 场景 | `USER.md` | `IDENTITY.md` | `SOUL.md` |
| --- | --- | --- | --- |
| 用户明确给出自己的称呼 | 写入用户称呼 | 不变 | 不变 |
| 用户选择"留白 / 后续养成" | 不变 | 不固定 AI 名字 | 使用默认 Soul，保留可养成状态 |
| 用户选择候选并沿用候选名 | 不变 | 写入候选名 | 写入对应预设模板 |
| 用户选择候选但改名 | 不变 | 写入用户改后的 AI 名字 | 写入对应预设模板 |
| 用户自由设定 AI 名字和人设 | 不变 | 写入明确 AI 名字 | 写入 LLM 整理后的自定义人设摘要 |
| 用户只给 AI 名字 | 不变 | 写入 AI 名字 | 保持默认 / 留白 |
| 用户只给人设 | 不变 | 不固定 AI 名字 | 写入预设模板或自定义人设摘要 |
| 用户回复不相关或低置信 | 不写 | 不写 | 不写 |

留白状态不是失败状态。它表示 `IDENTITY.md` 和 / 或 `SOUL.md` 的对应字段保持开放，后续正常对话中用户明确反馈 AI 称呼或人格期待时，可以通过用户显式纠正 / 自然养成链路写入。已明确选择的 AI 名字或预设人设应保持连续性，不因随口反馈被整体重置。
...
- 新用户 onboarding 需要额外装载默认身份、默认 Soul、两步流程状态、第二步候选列表和未完成设置项。
...
明文查看正文必须遵守 [运营与后台 PRD](../../products/zhaoxi/capabilities/admin_ops_prd.md)：管理员可在必要 debug 和事故排查时查看；普通后台用户需要管理员审批后的 2 小时临时明文权限；所有明文查看都记录操作日志。
