...
本文承接 [主动消息与提醒技术设计](proactive_messaging_design.md) 和 [主动消息与提醒 PRD](../../../products/zhaoxi/capabilities/proactive_prd.md)，定义内容邀请的详细技术方案。内容邀请不是订阅型内容推送，也不是到点自动发日报；主动阶段只能发送朋友式询问，用户正向确认后才在当前入站回合返回标题列表。
