...
- [`agent_context_files.md`](../../agent-runtime/agent_context_files.md)、[`dreaming_memory_design.md`](../../agent-runtime/dreaming_memory_design.md)、[`relationship_state_design.md`](relationship_state_design.md)
...
8. **[`docs/ops/products/zhaoxi/admin_guide.md`](../../../ops/products/zhaoxi/admin_guide.md#companion-world-p1-发布运行手册)**：生产模板导入、固定 cutoff backfill、对账、开关与回滚步骤。
...
- API/composition root：`app/bootstrap/http.py`、`app/products/zhaoxi/manifest.py`、
  `app/products/zhaoxi/api/*`；legacy `web.py` / `app_api.py` 仍在 `app/routers/`。
- 纯领域层：`app/products/zhaoxi/domain/{companion_world,missions}/`。
- 产品用例与持久化：`app/products/zhaoxi/application/*`、
  `app/products/zhaoxi/infrastructure/{persistence,repositories}/`。
- 主动消息与产品工具：`app/products/zhaoxi/proactive/*`、`app/products/zhaoxi/tools/*`。
- Runtime 接缝：`app/agent_runtime/{ports,adapter}.py`；现有 turn 核心仍在
  `app/turn_service.py`，属于显式过渡模块。
...
- 通知收件箱：`app/domains/companion_world/notifications.py`、`app/platform/app_inbox.py`、`app/routers/app_notifications.py`；per-resident App 路由位于 `app/products/zhaoxi/proactive/{contract/common,delivery/outbound}.py`。
...
- [x] **T3-3 DB helpers（M3-4）**：`app/products/zhaoxi/infrastructure/persistence/notifications.py` 已交付 visible 幂等写、all/unread tuple cursor、未读数、单条/read-all、7/30 天与 200 条 cleanup；所有 helper 强制 `platform_user_id` 锚，淘汰顺序为最旧 read → 最旧 unread，并保护当前事务新插入行。
...
- 生产开启前仍必须完成四模板 manifest、固定 cutoff backfill、客户端最低版本、客户端共享/legacy 豁免口径同步与双后端门禁；详见 [`docs/ops/products/zhaoxi/admin_guide.md`](../../../ops/products/zhaoxi/admin_guide.md#companion-world-p1-发布运行手册)。
