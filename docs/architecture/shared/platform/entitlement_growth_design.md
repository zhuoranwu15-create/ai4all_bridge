...
本文承接 [贝壳、增长与支付后置 PRD](../../../products/zhaoxi/capabilities/entitlement_growth_prd.md)，定义 Phase 1 内测所需的贝壳 wallet/ledger、用量计量、邀请奖励和客服补发技术设计。
...
- 搜索 provider、失败体验和成本细节，见 [Web Search 同步工具调用技术设计](../../agent-runtime/search_async_tasks_design.md)。
- 语音输入当前依赖上游转写，不产生后端 ASR 成本；细节见 [语音输入技术设计](../../products/zhaoxi/voice_input_design.md)。
