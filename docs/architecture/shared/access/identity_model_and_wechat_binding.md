...
> 2026-06-06 之后，账号标识和绑定口径以本文、`docs/architecture/overview.md` 和 `docs/architecture/system_design.md` 为准；早期文档里的 `account_id` 需要结合上下文判断。
...
- 已在插件源码工作区和本机运行时安装包补充 `gatewayMethods: ["web.login.start", "web.login.wait"]`，重启 Gateway 后，`web.login.start` 已能返回 `qrDataUrl`、`message`、`sessionKey`。补丁维护说明见 `docs/architecture/shared/access/openclaw_weixin_gateway_qr_patch.md`。
