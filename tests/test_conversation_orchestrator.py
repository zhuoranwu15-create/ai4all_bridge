...
def test_max_turn_rotation_carries_previous_context_into_next_prompt(client, test_settings):
    from app.db import ACCOUNT_ACTIVE_SESSION_KEY, list_sessions_for_account

    test_settings.conversation_session_max_turns = 1
    account_id = "sk-session-rotate"
    ...
    with patch(
        ...
        first = client.post(
            "/openclaw/turn",
            json=_turn_payload(
                channel_account_id="rotate-bot",
                openclaw_session_key=account_id,
                message_id="rotate-msg-1",
                text="第一轮上下文",
            ),
            headers=BRIDGE_HEADERS,
        )
    ...
    sessions = list_sessions_for_account(account_id=account_id, limit=10)
    assert len(sessions) == 2
    active = next(item for item in sessions if item["session_key"] == ACCOUNT_ACTIVE_SESSION_KEY)
    closed = next(item for item in sessions if item["id"] != active["id"])
    assert closed["status"] == "closed"
    assert closed["close_reason"] == "max_turns"
