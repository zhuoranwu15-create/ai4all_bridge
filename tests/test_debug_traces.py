...
def test_debug_trace_records_only_configured_accounts(client, fresh_db):
    ...
    with patch("app.main.generate_reply", return_value="debug reply") as mock_generate:
        res = client.post(
            "/openclaw/turn",
...
def test_failed_assistant_replies_are_excluded_from_llm_history(client):
    with patch("app.main.generate_reply", return_value="healthy reply") as mock_generate:
        res = client.post(
            "/openclaw/turn",
